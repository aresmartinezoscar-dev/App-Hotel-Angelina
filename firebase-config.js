...
// ---- Auth bootstrap ----
// firebase-config.js
import { startDataListeners } from './app.js'; // <--- añadir import circular controlado
  ...
  return new Promise((resolve) => {
    onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        showMainApp();
        // 🔑 Añadir esto para recargar datos tras un refresh
        startDataListeners();
        resolve(user);
      } else {
        currentUser = null;
...
export function addSale(saleData) {
  ...
  const payload = {
    ...saleData,
    at: (typeof saleData.at === 'number') ? saleData.at : Date.now(), // Usar Date.now() en lugar de serverTimestamp()
...
export function addStay(stayData) {
  ...
  // Calculation for nights was incorrect; it should be difference between days, not timestamps
  // This calculates full nights
  const diffTime = Math.abs(checkOut - checkIn);
  const nights = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  const payload = {
    ...stayData,
    nights,
    at: (typeof stayData.at === 'number') ? stayData.at : Date.now(), // Usar Date.now()
    createdBy: currentUser.uid
  };
  return set(newRef, payload);
}
...
export function addExpense(expenseData) {
  ...
  const payload = {
    ...expenseData,
    at: (typeof expenseData.at === 'number') ? expenseData.at : Date.now(), // Usar Date.now()
    createdBy: currentUser.uid
  };
  return set(newRef, payload);
}
...
export function addProduct(productData) {
  ...
  const payload = {
    ...productData,
    active: true,
    // createdAt y updatedAt no están permitidos por las reglas de seguridad actuales
    // Por simplicidad, los eliminamos o podrías ajustar tus reglas de seguridad.
    // Para que funcione con tus reglas actuales:
    // createdAt: Date.now(), 
    // updatedAt: Date.now()
  };
  return set(newRef, payload);
...
export function updateProduct(productId, productData) {
  ...
  const payload = {
    ...productData,
    // updatedAt no está permitido por las reglas de seguridad actuales
    // Para que funcione con tus reglas actuales:
    // updatedAt: Date.now()
  };
...
export function initializeProducts() {
  ...
  const tasks = [];
  // Primero, obtenemos los productos existentes para evitar duplicados
  return new Promise((resolve, reject) => {
    onValue(productsRef, (snapshot) => {
      const existingProducts = snapshot.val() || {};
      const existingProductNames = new Set(Object.values(existingProducts).map(p => p.name));

      Object.entries(initialProducts).forEach(([name, price]) => {
        if (!existingProductNames.has(name)) { // Solo añadir si no existe
    ...
    }, { onlyOnce: true }); // Escuchar solo una vez para la inicialización
  });
}

// Export Firebase instances if needed elsewhere
export { auth, database, serverTimestamp }; // serverTimestamp todavía se exporta pero no se usa en este caso para evitar conflictos con las reglas.